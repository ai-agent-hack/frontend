...
*少々お待ちください！*`;
                ...
                const pins: MapPin[] =
                    spots.recommend_spots.recommend_spots.flatMap((timeSlot) =>
    ...
    const handleRecommendSpotUpdate = useCallback(
        (recommendSpot: RecommendedSpots) => {
            ...
            const pins: MapPin[] = recommendSpot.recommend_spots.flatMap(
                (timeSlot) =>
                    timeSlot.spots.map((spot, index) => ({
                        id: `${timeSlot.time_slot}-${spot.spot_id}-${index}`,
    ...
    const handleSpotSelect = useCallback(
        (spotId: string, isSelected: boolean) => {
            ...
            const pins: MapPin[] = updatedSpots.recommend_spots.flatMap(
                (timeSlot) =>
                    timeSlot.spots.map((spot, index) => ({
                        id: `${timeSlot.time_slot}-${spot.spot_id}-${index}`,
                        position: { lat: spot.latitude, lng: spot.longitude },
            ...
            <HStack height="100%" gap={"15px"} position="relative">
                ...
                <Box
                    ...
                    <GoogleMap
                        ...
                        pins={mapPins.filter((pin) =>
                            pin.id.startsWith(selectedTimeSlot)
                        )}
                    ...
                    <Box
                        ...
                        <Button
                            ...
                            transition="all 0.2s"
                            disabled={
                                !mapPins.some((pin) => pin.selected) || isSaving
                            }
                            onClick={async () => {
