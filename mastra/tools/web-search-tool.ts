...
export const webSearchTool = createTool({
  ...
  inputSchema: z.object({
    ...
    summaryOnly: z
      .boolean()
      .default(true)
      .describe("Return only search summaries without full content scraping"),
  }),
  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      if (summaryOnly) {
        const summaryParts = (searchResults.data || [])
          .slice(0, limit)
      ...
      const scrapePromises = (searchResults.data || [])
        .slice(0, limit)
        .map(async (result: any) => {
          try {
            ...
            if (scrapedData.success && "markdown" in scrapedData) {
              const title =
                result.title || scrapedData.metadata?.title || "No title";
      ...
      const markdownParts = results.filter(
        (result) => result !== null,
      ) as string[];
