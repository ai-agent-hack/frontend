...
const checkIntentStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    let intentType:
      ...
      | "general_chat"
      | "spot_detail"
      | "route_creation_execute" = "general_chat";
    ...
    if (userInput === "旅行ルート作成を開始して") {
      ...
      intentType = intentResult.object.isSpotSearch
        ? "spot_search"
        : "general_chat";
    }
...
const nonSpotResponseStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    return {
      message:
        result.text ||
        "申し訳ございません。お手伝いできることがありましたらお申し付けください。",
    };
  },
});
...
const spotDetailStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    try {
      ...
      const explanationAgent = mastra.getAgent(
      ...
      return {
        message:
          ...
          `${reviewData.place_name}についての詳細情報です。`,
      };
    } catch (error) {
      console.error("スポット詳細取得エラー:", error);
      return {
        message:
          "スポット情報の取得中にエラーが発生しました。もう一度お試しください。",
      };
    }
  },
});
...
const routeCreationExecuteStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    // 選択されているスポットを確認
    const selectedSpots =
      recommendSpotObject?.recommend_spots?.flatMap((timeSlot: any) =>
    ...
    if (selectedSpots.length === 0) {
      return {
        message:
          "スポットが選択されていません。地図上のピンをクリックして、お気に入りのスポットを選んでください。",
        recommendSpotObject: recommendSpotObject || {
          recommend_spot_id: "",
          recommend_spots: [],
        },
    ...
    try {
      ...
      if (orderedSpots.length === 0) {
        return {
          message:
            "1日では回りきれないスポット数でした。もう少しスポットを減らしてみてください！",
        };
      }

      // conciseRouteExplanationAgentを使用して簡潔で魅力的なルート説明を生成
      const routeExplanationAgent = mastra.getAgent(
...
${orderedSpots.map((spot: any, index: number) => `${index + 1}. ${spot.name}${spot.time ? ` (${spot.time})` : ""}`).join("\n")}`,
      ...
      return {
        message:
          routeExplanationResult.text ||
          "旅行ルートを作成しました！選択されたスポットを効率的に巡れるルートです。",
        polyline: polyline,
...
export const recommendSpotWorkflow = createWorkflow({
  ...
  .branch([
    ...
    [
      async ({ inputData }) =>
        inputData.intentType === "route_creation_execute",
      routeCreationExecuteStep,
    ],
    // スポット検索の場合
    [
      async ({ inputData }) => inputData.intentType === "spot_search",
      spotSearchChain,
    ],
    // 一般的なチャットの場合
    [
      async ({ inputData }) => inputData.intentType === "general_chat",
      nonSpotResponseStep,
    ],
    // スポット詳細の場合
    [
      async ({ inputData }) => inputData.intentType === "spot_detail",
      spotDetailStep,
    ],
  ])
  .commit();
